    ...
    <header className="relative bg-white">
      ...
      <div className="bg-blue-800 text-white">
        <div className="container mx-auto flex justify-between items-center py-4 px-4">
          ...
          <div className="absolute left-1/2 -translate-x-1/2">
            <Link href="/" className="font-extrabold text-3xl sm:text-5xl">
              Online Kanda
            </Link>
          </div>
      ...
      <div
        className={`hidden lg:block border-b transition-all duration-300 ${
          isSticky
            ? "fixed top-0 left-0 right-0 z-40 shadow-lg bg-white"
            : "relative bg-white"
        }`}
      >
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link}
                href={href}
                className={`text-gray-700 font-semibold uppercase tracking-wider text-sm pb-1 hover:text-blue-600 ${
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={href}
                  onClick={() => setIsMenuOpen(false)}
                  className="text-gray-800 font-bold text-lg py-2 border-b"
                >
                  {link}
                </Link>
              );
